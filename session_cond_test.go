...
func TestBuilder(t *testing.T) {
	...
	var cond Condition
	has, err := testEngine.Where(phoenixormbuilder.Eq{"col_name": "col1"}).Get(&cond)
	assert.NoError(t, err)
	assert.Equal(t, true, has, "records should exist")

	has, err = testEngine.Where(phoenixormbuilder.Eq{"col_name": "col1"}.
		And(phoenixormbuilder.Eq{"op": OpEqual})).
		NoAutoCondition().
		Get(&cond)
	assert.NoError(t, err)
	assert.Equal(t, true, has, "records should exist")

	has, err = testEngine.Where(phoenixormbuilder.Eq{"col_name": "col1", "op": OpEqual, "value": "1"}).
		NoAutoCondition().
		Get(&cond)
	assert.NoError(t, err)
	assert.Equal(t, true, has, "records should exist")
	...
	has, err = testEngine.Where(phoenixormbuilder.Eq{"col_name": "col1"}.
		And(phoenixormbuilder.Neq{"op": OpEqual})).
		NoAutoCondition().
		Get(&cond)
	assert.NoError(t, err)
	...
	var conds []Condition
	err = testEngine.Where(phoenixormbuilder.Eq{"col_name": "col1"}.
		And(phoenixormbuilder.Eq{"op": OpEqual})).
		Find(&conds)
	assert.NoError(t, err)
	...
	err = testEngine.Where(phoenixormbuilder.Like{"col_name", "col"}).Find(&conds)
	assert.NoError(t, err)
	assert.EqualValues(t, 1, len(conds), "records should exist")

	conds = make([]Condition, 0)
	err = testEngine.Where(phoenixormbuilder.Expr("col_name = ?", "col1")).Find(&conds)
	assert.NoError(t, err)
	assert.EqualValues(t, 1, len(conds), "records should exist")

	conds = make([]Condition, 0)
	err = testEngine.Where(phoenixormbuilder.In("col_name", "col1", "col2")).Find(&conds)
	assert.NoError(t, err)
	assert.EqualValues(t, 1, len(conds), "records should exist")
	...
	// complex condtions
	var where = phoenixormbuilder.NewCond()
	if true {
		where = where.And(phoenixormbuilder.Eq{"col_name": "col1"})
		where = where.Or(phoenixormbuilder.And(phoenixormbuilder.In("col_name", "col1", "col2"), phoenixormbuilder.Expr("col_name = ?", "col1")))
	}
