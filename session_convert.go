...
func (session *Session) str2Time(col *phoenixormcore.Column, data string) (outTime time.Time, outErr error) {
	...
	if sdata == zeroTime0 || sdata == zeroTime1 {
	...
	} else if col.SQLType.Name == phoenixormcore.Time {
		...
		if session.engine.dialect.DBType() == phoenixormcore.MYSQL && len(sdata) > 8 {
...
func (session *Session) byte2Time(col *phoenixormcore.Column, data []byte) (outTime time.Time, outErr error) {
...
func (session *Session) bytes2Value(col *phoenixormcore.Column, fieldValue *reflect.Value, data []byte) error {
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		...
		if col.SQLType.Name == phoenixormcore.Bit &&
			session.engine.dialect.DBType() == phoenixormcore.MYSQL { // !nashtsai! TODO dialect needs to provide conversion interface API
			if len(data) == 1 {
				x = int64(data[0])
			} else {
				x = 0
	...
	case reflect.Ptr:
		...
		// case "*string":
		case phoenixormcore.StringType.Kind():
			x := string(data)
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*bool":
		case phoenixormcore.BoolType.Kind():
			d := string(data)
			...
			fieldValue.Set(reflect.ValueOf(&v).Convert(fieldType))
		// case "*complex64":
		case phoenixormcore.Complex64Type.Kind():
			var x complex64
			if len(data) > 0 {
		...
		// case "*complex128":
		case phoenixormcore.Complex128Type.Kind():
			var x complex128
			if len(data) > 0 {
				err := DefaultJSONHandler.Unmarshal(data, &x)
				if err != nil {
		...
		// case "*float64":
		case phoenixormcore.Float64Type.Kind():
			x, err := strconv.ParseFloat(string(data), 64)
			if err != nil {
				return fmt.Errorf("arg %v as float64: %s", key, err.Error())
			}
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*float32":
		case phoenixormcore.Float32Type.Kind():
			var x float32
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*uint64":
		case phoenixormcore.Uint64Type.Kind():
			var x uint64
			x, err := strconv.ParseUint(string(data), 10, 64)
			if err != nil {
				return fmt.Errorf("arg %v as int: %s", key, err.Error())
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*uint":
		case phoenixormcore.UintType.Kind():
			var x uint
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*uint32":
		case phoenixormcore.Uint32Type.Kind():
			var x uint32
			x1, err := strconv.ParseUint(string(data), 10, 64)
			if err != nil {
				return fmt.Errorf("arg %v as int: %s", key, err.Error())
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*uint8":
		case phoenixormcore.Uint8Type.Kind():
			var x uint8
			x1, err := strconv.ParseUint(string(data), 10, 64)
			if err != nil {
				return fmt.Errorf("arg %v as int: %s", key, err.Error())
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*uint16":
		case phoenixormcore.Uint16Type.Kind():
			var x uint16
			x1, err := strconv.ParseUint(string(data), 10, 64)
			if err != nil {
				return fmt.Errorf("arg %v as int: %s", key, err.Error())
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*int64":
		case phoenixormcore.Int64Type.Kind():
			sdata := string(data)
			var x int64
			var err error
			// for mysql, when use bit, it returned \x01
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*int":
		case phoenixormcore.IntType.Kind():
			sdata := string(data)
			var x int
			var x1 int64
			var err error
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*int32":
		case phoenixormcore.Int32Type.Kind():
			sdata := string(data)
			var x int32
			var x1 int64
			var err error
			// for mysql, when use bit, it returned \x01
			if col.SQLType.Name == phoenixormcore.Bit &&
				session.engine.dialect.DBType() == phoenixormcore.MYSQL {
				if len(data) == 1 {
					x = int32(data[0])
				} else {
					x = 0
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*int8":
		case phoenixormcore.Int8Type.Kind():
			sdata := string(data)
			var x int8
			var x1 int64
			var err error
			...
			fieldValue.Set(reflect.ValueOf(&x).Convert(fieldType))
		// case "*int16":
		case phoenixormcore.Int16Type.Kind():
			sdata := string(data)
			var x int16
			var x1 int64
			var err error
...
func (session *Session) value2Interface(col *phoenixormcore.Column, fieldValue reflect.Value) (interface{}, error) {
	if fieldValue.CanAddr() {
		if fieldConvert, ok := fieldValue.Addr().Interface().(phoenixormcore.Conversion); ok {
	...
	if fieldConvert, ok := fieldValue.Interface().(phoenixormcore.Conversion); ok {
		data, err := fieldConvert.ToDB()
		if err != nil {
			return 0, err
		}
